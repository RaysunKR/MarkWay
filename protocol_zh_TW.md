...
| 模式 | 根請求地址 | 協議文件位置 | 內容要求 |
|------|-----------|-------------|---------|
| 靜態模式 | `/index.md` | `/protocol.md`（固定） | 本協議任意語言版本的 README.md 內容 |
| 動態模式 | `/` | `/protocol`（固定） | 本協議任意語言版本的 README.md 內容 |
...
- 所有 `index.md` 及通過 GET 請求返回的 Markdown 內容，必須在開頭聲明本站使用 MarkWay 協定並給出 protocol 地址
  - 文件/子目錄名稱
  - 地址（支援三種路徑類型）
  - 主要內容
  - 作用說明
...
**規則：**
- **GET 請求**：返回 Markdown 格式的介面文件，URL 為絕對靜態路徑，**禁止使用 URL 傳參**。回應內容必須在開頭聲明本站使用 MarkWay 協定並給出本站 protocol 地址
